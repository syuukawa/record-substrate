...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn bid_auction(origin, kitty_id: T::Hash, bid: T::Balance) -> Result {
            ...
            <Bids<T>>::insert((kitty_id, sender.clone()), bid); //竞拍代币的托管
        ...
        fn on_finalize() {
            ...
            for auction in &auctions {
                ...
                let bid_accounts = Self::bid_accounts(auction.kitty_id); //获取所有的竞拍者

                // ACTION: for each account,
                //      unreserve the balance
                //      remove from <Bids>
                for account in bid_accounts {
                    let bid_balance = Self::bid_of((auction.kitty_id, account.clone())); //获取竞拍者托管的代币数量
