use support::{decl_storage, decl_module, StorageValue, StorageMap,
    dispatch::Result, ensure, decl_event, traits::Currency, traits::ReservableCurrency};
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn bid_auction(origin, kitty_id: T::Hash, bid: T::Balance) -> Result {
            ...
            // ACTION: ensure the kitty has an existing auction : 拍卖是否存在
            ensure!(<KittyAuction<T>>::exists(kitty_id), "This auction does not exist");

            // ACTION: ensure the existing auction is not expired
            // 1-获取auction竞拍时间，和当前区块高度进行比较
            ...
            if <Bids<T>>::exists((kitty_id, sender.clone())) { //判断竞拍者是否参与过竞拍
