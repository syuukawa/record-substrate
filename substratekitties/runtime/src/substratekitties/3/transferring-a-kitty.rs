...
impl<T: Trait> Module<T> {
    ...
    fn transfer_from(from: T::AccountId, to: T::AccountId, kitty_id: T::Hash) -> Result {
        ...
        let owner = Self::owner_of(kitty_id).ok_or("No owner for this kitty")?; //最后的？
        ...
        let new_owned_kitty_count_to = owned_kitty_count_to.checked_add(1)
            .ok_or("Overflow adding a new kitty to the AccountId")?;
        let new_owned_kitty_count_from = owned_kitty_count_from.checked_sub(1)
            .ok_or("underflow subing a new kitty from the AccountId")?;
        ...
        <KittyOwner<T>>::insert(&kitty_id, &to);
        <OwnedKittiesIndex<T>>::insert(kitty_id, owned_kitty_count_to);//count-1
