...
decl_storage! {
    trait Store for Module<T: Trait> as KittyStorage {
        ...
        OwnedKittiesArray get(kitty_of_owner_by_index): map (T::AccountId, u64) => T::Hash; //index和id的对应关系
        // ACTION: Add a new storage item `OwnedKittiesCount` which is a `map` from `T::AccountId` to `u64`
        OwnedKittiesCount get(owned_kitties_count): map T::AccountId=> u64; //所有者拥有kitty的数量
        // ACTION: Add a new storage item `OwnedKittiesIndex` which is a `map` from `T::Hash` to `u64`
        OwnedKittiesIndex get(owned_kitties_index): map T::Hash => u64;  //id和index的对应关系
